...
public class Program
{
    ...
    class FunctionContainer : CustomType
    {
        ...
        public int test(object[] array)
        {
            return 4;
        }
    }

    class CustomReadOnlyList : IReadOnlyList<string>, IIterable
    {
        private readonly string[] _values = new[] { "a", "b", "c" };
        public int Count => _values.Length;
        public string this[int index] => _values[index];
        public IEnumerator<string> GetEnumerator() => _values.AsEnumerable().GetEnumerator();
    ...
    private static void testEx()
    {
        ...
        //context.Eval("function test({a, b}) { return a; } console.log(test({a:1,b:2}))");

        //context.Eval("const f = (a) => a");
        //var result = context.Eval("({l : s = 1, x, ...r} = {x : 2, y : 3}); r");
        //var result = context.Eval("([s = 1,, x, ...{ pop, ...r }] = [, 2, 3]); r");
        //context.Eval("var {s = 1, x} = {x : 1};");// const f = (a, [b, c = 2] , d, {e = 1, f }) => a");

        //context.Eval("const SvgPause = ({\r\n  svgRef,\r\n  ...props\r\n}) => react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(\"svg\", _extends({\r\n  width: \"16px\",\r\n  height: \"20px\",\r\n  viewBox: \"0 0 16 20\",\r\n  ref: svgRef\r\n}, props), _ref, _ref2, _ref3, _ref4);");
        ...
        context.Eval("console.log(Math.round(2285020100));");

        return;

        context.DefineVariable("list").Assign(context.GlobalContext.WrapValue(new CustomReadOnlyList()));

        context.Eval("for (var x of list) console.log(x); list.Do();");

        return;

        var sm = new StringMap<object>
        {
            ["123"] = 123,
            ["hello"] = "hello",
        };

        Console.WriteLine(string.Join(',', sm.Select(x => x.Key + ":" + x.Value)));
    ...
    static readonly Dictionary<int, bool> _timeoutsActive = new();
    ...
    static void Main(string[] args)
    {
        ...
        Context.DefaultGlobalContext.DefineVariable("$").Assign(Context.CurrentGlobalContext.ProxyValue(
            new
            {
        ...
        Context.DefaultGlobalContext.DefineVariable("setTimeout").Assign(Context.CurrentGlobalContext.ProxyValue((ICallable callback, int delay) =>
        {
            lock (_timeoutsActive)
            {
                var id = Random.Shared.Next();
                while (_timeoutsActive.ContainsKey(id))
                    id++;

                _timeoutsActive[id] = true;

                Task.Delay(delay).ContinueWith(_ => { lock (_timeoutsActive) _timeoutsActive.Remove(id); callback.Call(null, null); });
...
#if !PORTABLE
        Context.DefaultGlobalContext.DefineVariable("$nil").Assign(Context.CurrentGlobalContext.ProxyValue(
            new
            {
                GetCtor = new Func<string, JSValue>(name => Context.CurrentGlobalContext.GetConstructor(NamespaceProvider.GetType(name)))
            }));
#endif
...
export default class Test {
    ...
    constructor(private ss: SampleStore) {
                ...
                System.Diagnostics.Debug.Assert(@"import { SampleStore } from 'utiliread';
export default class Test {
    constructor(ss) {
        this.ss = ss;
    }
...
" == transpiled);
            ...
            case 1:
            {
                // runFiles("custom/");
                // webkitTests();
                runFile("C:\\Users\\Дмитрий\\Documents\\Visual Studio 2017\\Projects\\NiL.JS\\Sandbox\\functions.js");
                break;
            }
            case 2:
            {
            ...
            case 5:
            {
                ...
                //runFile("linq.js", 65536 * 2);
                //runFile("dromaeo/tests/evaluation.js", 65536 * 8);
            ...
            case 12:
            {
                runTestFile(@"tests\sputnik\ch15\15.1\15.1.3\15.1.3.1\S15.1.3.1_A2.5_T1.js");
    ...
    private static void dromaeoTests()
    {
        ...
        var tests = Directory.EnumerateFiles(rootPath + "tests\\", "*stress.js", SearchOption.AllDirectories)
            //.Select(x => new Module(x, File.ReadAllText(x)))
            .Select(x => (path: x, code: File.ReadAllText(x), module: new Module(x, File.ReadAllText(x))))
            .ToArray();

        string dromaeoHelpers = @"
        var startTest = function () { };
        var test = function (name, fn) { fn(); };
        var endTest = function () { };
        var prep = function (fn) { fn(); };";

        tests.All(x => x.module.Context.Eval(runner) != null);
        ...
        for (; round < 100; round++)
        {
            ...
            {
                _("Process " + tests[i].path);

                sw.Restart();
                new Context(true).Eval(dromaeoHelpers + Environment.NewLine + Environment.NewLine + tests[i].code);
                //tests[i].module.Run();
                sw.Stop();
...
");
    ...
    private static void runTestFile(string filename)
    {
        ...
        Console.WriteLine("-------------------------------------");
        sw.Restart();
        Script.Parse(sr.ReadToEnd()).Evaluate(s.Context);
        sw.Stop();
        Console.WriteLine("-------------------------------------");
    ...
    private static void sunspider()
    {
        ...
        var tests = Directory.EnumerateFiles(rootPath, "*.js", SearchOption.AllDirectories)
            //.Where(x => x.Contains("cordic"))
            .Select(x => new Module(x, File.ReadAllText(x), options: Options.None))
            .ToArray();
