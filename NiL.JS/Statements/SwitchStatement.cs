...
namespace NiL.JS.Statements
{
    ...
    public sealed class SwitchStatement : CodeNode
    {
        ...
        internal override bool Build(ref CodeNode _this, int depth, Dictionary<string, VariableDescriptor> variables, bool strict, CompilerMessageCallback message, FunctionStatistic statistic)
        {
            if (depth < 1)
                throw new InvalidOperationException();
            Parser.Build(ref image, 2, variables, strict, message, statistic);
            for (int i = 0; i < lines.Length; i++)
                Parser.Build(ref lines[i], 1, variables, strict, message, statistic);
            for (int i = 0; functions != null && i < functions.Length; i++)
            {
                CodeNode stat = functions[i];
                Parser.Build(ref stat, 1, variables, strict, message, statistic);
                variables[functions[i].Name] = new VariableDescriptor(functions[i].Reference, true, functions[i].Reference.functionDepth);
            }
            functions = null;
            for (int i = 1; i < cases.Length; i++)
                Parser.Build(ref cases[i].statement, 2, variables, strict, message, statistic);
