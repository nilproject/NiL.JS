...
namespace NiL.JS.Statements
{
    ...
    internal class OperatorStatement : Statement, IOptimizable
    {
        ...
        {
            return Parse(code, ref index, true, false);
        }

        public static ParseResult Parse(string code, ref int index, bool processComma)
        {
            return Parse(code, ref index, processComma, false);
        }

        private static ParseResult Parse(string code, ref int index, bool processComma, bool forUnary)
        {
            int i = index;
                    ...
                    case '+':
                        {
                            ...
                            if (code[i] == '+')
                            {
                                ...
                                first = Parse(code, ref i, true, true).Statement;
                                (first as OperatorStatement).type = OperationType.Incriment;
                            }
                            else
                            {
                                var f = Parser.Parse(code, ref i, 1);
                                index = i;
                    ...
                    case '-':
                        {
                            ...
                            else
                            {
                                ...
                                first = new OperatorStatement() { first = new ImmidateValueStatement(0), second = Parse(code, ref i, true, true).Statement, type = OperationType.Substract };
                            }
                            break;
                        }
                    case '!':
                    ...
                    case 't':
                        {
                            i += 5;
                            do i++; while (char.IsWhiteSpace(code[i]));
                            first = Parse(code, ref i, false, true).Statement;
                    ...
                    case 'v':
                        {
                            i += 3;
                            do i++; while (char.IsWhiteSpace(code[i]));
                            first = new OperatorStatement() { first = new ImmidateValueStatement(JSObject.undefined), second = Parse(code, ref i, false, true).Statement, type = OperationType.None };
                            break;
                        }
                    case 'n':
                        {
            ...
            {
                i++;
                first = OperatorStatement.Parse(code, ref i, true).Statement;
                if (code[i] != ')')
                    throw new ArgumentException();
                i++;
            }
            else
                first = Parser.Parse(code, ref i, 2);
                    ...
                    case ',':
                        {
                            ...
                            for (; ; )
                            {
                                ...
                                args.Add(OperatorStatement.Parse(code, ref i, false).Statement);
                            }
                    ...
                    case '[':
                        {
                            ...
                            for (; ; )
                            {
                                ...
                                if (code[i] == ']')
                                    ...
                                    do i++; while (char.IsWhiteSpace(code[i]));
                                args.Add(Parser.Parse(code, ref i, 1));
            ...
            {
                do i++; while (char.IsWhiteSpace(code[i]));
                second = OperatorStatement.Parse(code, ref i, processComma).Statement;
        ...
        private unsafe JSObject OpAnd(Context context)
        {
            ...
            if (lvt == ObjectValueType.Int)
            {
                ...
                if (temp.ValueType == ObjectValueType.Int)
                    ...
                    tempResult.iValue = (int)left & (int)(temp.bValue ? 1 : 0);
            }
            ...
            else if (lvt == ObjectValueType.Bool)
            {
        ...
        private JSObject OpNone(Context context)
        {
            if (second != null)
            {
        ...
        private unsafe JSObject OpNotEqual(Context context)
        {
            var temp = first.Invoke(context);
        ...
        private unsafe JSObject OpNot(Context context)
        {
            ...
            if (vt == ObjectValueType.Int)
                o.iValue = val.iValue ^ -1;
            else if (vt == ObjectValueType.Double)
                o.iValue = (int)val.dValue ^ -1;
            else if (vt == ObjectValueType.Bool)
                o.iValue = (val.bValue ? -2 : -1);
            else
                o.iValue = -1;
            return o;
        }
        ...
        private unsafe JSObject OpLogicalNot(Context context)
        {
            ...
            o.ValueType = ObjectValueType.Bool;
            if (vt == ObjectValueType.Int)
                o.bValue = val.iValue == 0;
            else if (vt == ObjectValueType.Double)
                o.bValue = val.dValue == 0.0;
            else if (vt == ObjectValueType.Bool)
                o.bValue = !val.bValue;
            else if (vt == ObjectValueType.String)
                o.bValue = string.IsNullOrEmpty(val.oValue as string);
            else if (vt == ObjectValueType.Object)
                o.bValue = val.oValue == null;
            else throw new NotImplementedException();
            return o;
        }
        ...
        public bool Optimize(ref Statement _this, int depth, HashSet<string> vars)
        {
            ...
            else
            {
                ...
                if ((type == OperationType.None) && (second == null))
                {
                    _this = first;
                    return true;
                }
