...
namespace NiL.JS.Core
{
    ...
    public class Context : IEnumerable<string>
    {
        ...
        public static void RefreshGlobalContext()
        {
            ...
            try
            {
                ...
                foreach (var v in globalContext.fields.Values)
                    v.attributes |= JSObjectAttributesInternal.DoNotEnumerate;
            }
            catch
            {
                throw;
        ...
        public void AttachModule(Type moduleType)
        {
            ...
            fields[name].attributes = JSObjectAttributesInternal.DoNotEnumerate;
        }
                        ...
                        for (i = context.variables.Length; i-- > 0; )
                        {
                            ...
                            if (vars.TryGetValue(context.variables[i].name, out desc))
                            {
                                if (desc.IsDefined)
                                {
                                     ...
                                     *  function b(){
                                     *      eval("var d = 2");
                                     *      // переменная объявлена в контексте d, значит и значение должно быть из
                                     *      // контекста d, но если по выходу из d кэш этой переменной сброшен не будет, 
